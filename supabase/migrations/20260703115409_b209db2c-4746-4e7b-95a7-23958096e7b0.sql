...
ALTER TABLE public.tenants
  ADD COLUMN IF NOT EXISTS rent_due_month_offset integer NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.tenants.rent_due_month_offset IS
  'Offset in months from the billing (rent) month to the due date. -1 = advance (previous month), 0 = same month, 1 = arrears (following month).';
...
BEGIN
  ...
  LOOP
    -- Compute due date by shifting billing month by the tenant's offset
    ...
    IF NOT EXISTS (
      ...
      INSERT INTO rent_payments (
        tenant_id, property_id, unit_id, amount, due_date, status, billing_month
      ) VALUES (
      ...
      IF NOT EXISTS (
        ...
        INSERT INTO invoices (
          invoice_number, property_id, tenant_id, amount, due_date, status, created_by, items
        ) VALUES (
